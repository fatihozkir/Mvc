...
namespace Microsoft.AspNetCore.Mvc.TagHelpers
{
    ...
    public class ScriptTagHelper : UrlResolutionTagHelper
    {
        ...
        /// Value defaults to <c>true</c> if not provided.
        /// </summary>
        [HtmlAttributeName(FallbackIntegrityCheckAttributeName)]
        public bool FallbackIntegrityCheck { get; set; } = true;
            ...
            if (fallbackSrcs.Count > 0)
            {
                ...
                foreach (var src in fallbackSrcs)
                {
                    ...
                    for (var i = 0; i < attributes.Count; i++)
                    {
                        ...
                        {
                            // Do not write integrity attribute when the fallback integrity check is false
                            if (!FallbackIntegrityCheck && string.Equals(IntegrityAttributeName, attribute.Name, StringComparison.OrdinalIgnoreCase))
                            {
                                continue;
                            }
